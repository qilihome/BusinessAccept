...
public class BusinessInfoServiceImpl implements IBusinessInfoService
{
	...
	public BusinessInfo update(BusinessInfo businessInfo) throws Exception
	{
		...
		{
			throw new ServiceRulesException(
					String.format(Constant.REQUEST_ERROR, "修改业务信息", status));
		}
			...
			throws Exception
	{
		...
		// post
				...
				{
					list = (List<BusinessInfoVo>)JsonHepler.parseCollection(result, ArrayList.class, BusinessInfoVo.class);
				}
				else
				{
	...
	@Override
	public List<BusinessInfoVo> queryToMyBusinessInfo(int operatorID)
			throws Exception
	{
		List<BusinessInfoVo> list = null;
		// post
				...
				HttpClient client = new DefaultHttpClient(conn, params);
				HttpPost post = new HttpPost(uri);
				NameValuePair businessInfoJson = new BasicNameValuePair("operatorID",
						operatorID + "");
				List<NameValuePair> paramters = new ArrayList<NameValuePair>();
				paramters.add(businessInfoJson);
				post.setEntity(new UrlEncodedFormEntity(paramters, HTTP.UTF_8));
				...
				{
					throw new ServiceRulesException(
							String.format(Constant.REQUEST_ERROR, "查询待办业务信息", status));
				}
